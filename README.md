...
### _Sugoi Custom order entry and order tracking web application_
...
### _`/_` - secured - must be authenticated\*

- this route fowards all incoming requests not found in routes below to `Vue.js` frontend order confirmation

### _`/orders` - All routes secured - must be authenticated_

- `GET /all` - fetches all from `orders` collection regardless of status and displays in `index.handlebars` table.
- `GET /` - fetches all from `orders` collection in any pre-production status and displays in `index.handlebars` table.
...
### _`/payments` - All routes secured - must be authenticated and edit orders_
...
### _`/prod` - All routes secured - must be authenticated and edit prod_
...
### _`/proofs` - All routes secured - must be authenticated_

- `GET /uploadform` - _currently unavailable - coming in future build_.
- `POST /uploadform` - _currently unavailable - coming in future build_.
...
### _`/reports` - All routes secured - must be authenticated and admin_
...
### _`/styles` - currently unavailable, moved to API routes_

### _`/users`- All routes secured - must be authenticated and admin_
...
- _`/Home` - secured - must be authenticated_
...
- _`/EditDetails` - secured - must be authenticated_
...
- _`/EditLine` - secured - must be authenticated_

  - allows user to edit each line level details of the order individually
  - automatically updates and saves to state and updates the database once commited

- _`/EditItem` - secured - must be authenticated_
  - allows user to edit each item level details of the order individually
  - automatically updates and saves to state and updates the database once commited

_All changes made on order confirmation update the PO, XML file and order routes on the Backend._
