...
// SEC - MUST BE LOGGED IN
router.get('/xml/:orderNum', ensureAuthenticated, async (req, res) => {
  try {
  ...
  try {
    ...
    if (foundOrder.currentStatus === 'U. Uploaded') {
      ...
      if (foundOrder.uploadDate === null) {
        ...
        await Report.updateOne({ _id: updatedReport._id }, { $set: { avgOutput: outputAvg } });
      }
    }
...
// SEC - MUST BE LOGGED IN - MUST HAVE EDIT ORDERS ACCESS
router.put('/note-edit/:noteid', [ensureAuthenticated, ensureEditOrders], async (req, res) => {
  try {
    let { instruction, isr, instructionType } = req.body;
    let foundOrder = await Order.findOne({ 'instructions._id': req.params.noteid });
...
// SEC - MUST BE LOGGED IN - MUST HAVE EDIT ORDERS ACCESS
router.put('/revision/:id', [ensureAuthenticated, ensureEditOrders], async (req, res) => {
  try {
    let { instruction, isr } = req.body;
    if (!instruction) {
    ...
    foundOrder.currentStatus = 'G. Waiting for Revision';
    foundOrder.currentArtist = '';
    foundOrder.revisionRequestDate = dayjs().format();
    ...
    await foundOrder.save();
    logger.info(`${foundOrder.orderNum} - revision request by ${req.user.username}`);
...
// @DESC - EDITS NOTES BY PUSHING NEW ONE TO ARRAY BASED ON ORDER ID#
// SEC - MUST BE LOGGED IN - MUST HAVE EDIT ORDERS ACCESS
router.put('/notes/:id', [ensureAuthenticated, ensureEditOrders], async (req, res) => {
  try {
    let { instruction, noteUser } = req.body;
...
function getDateRangeOfWeek(weekNo, y) {
  ...
  rangeIsFrom = months[d1.getMonth()] + '-' + d1.getDate() + '-' + d1.getFullYear();
  d1.setDate(d1.getDate() + 6);
  rangeIsTo = months[d1.getMonth()] + '-' + d1.getDate() + '-' + d1.getFullYear();
