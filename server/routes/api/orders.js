...
// SEC - MUST BE LOGGED IN
router.put('/:orderNum/:lineNumber', [ensureAuthenticated, ensureEditOrders], async (req, res) => {
  try {
...
// SEC - MUST BE LOGGED IN
router.put('/:orderNum/:lineNumber/:itemNumber', [ensureAuthenticated, ensureEditOrders], async (req, res) => {
  try {
    let foundOrder = await Order.findOne({ orderNum: req.params.orderNum });
