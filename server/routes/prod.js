  ...
  Order.findOne({ _id: id }, function(err, foundOrder) {
    if (err) {
      ...
      if (vendorConfirmShip && sentVendor) {
        ...
        let date1 = moment(new Date(vendorConfirmShip)).format;
        let date2 = moment(new Date(sentVendor)).format;
      ...
      if (confirmDeliveryDate && vendorConfirmShip) {
        let date1 = moment(new Date(confirmDeliveryDate)).format;
        let date2 = moment(new Date(vendorConfirmShip)).format;
        let diff = new DateDiff(date1, date2);
