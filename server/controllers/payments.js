...
module.exports = {
		...
		try {
			...
			if (req.user.lgUser) {
				...
				orders = await Order.find({
					currentStatus: {
						$nin: ['W. CANCELLED', 'X. Archived', '1. Initial']
					},
					lgOrder: true
				});
			} else {
	...
	// @access  Private - Edit Orders Role
	getOutstandingPreProd: async (req, res, next) => {
		try {
	...
	// @access  Private - Edit Orders Role
	getOutstandingProd: async (req, res, next) => {
		try {
	...
	// @access  Private - Edit Orders Role
	getOutstandingShipped: async (req, res, next) => {
		try {
	...
	// @access  Private - Edit Orders Role
	getPaymentsEditPage: async (req, res, next) => {
		try {
	...
	// @access  Private - Edit Orders Role
	getPaymentLogs: async (req, res, next) => {
		try {
			const order = await Order.findOne({ _id: req.params.id });
			let logs = [];
