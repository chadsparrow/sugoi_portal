<template>
	<div v-if="cancelled === false">
		...
		<hr />
		<div
			class="card border-secondary mb-3 line-card"
			:id="orderLine.lineNumber"
		>
			<div class="card-header bg-secondary text-light p-1 justify-items-center">
				...
				<div
					...
					v-if="
						orderLine.graphicCode != 'CUSTM' && orderLine.graphicCode != null
					"
				>
					Quick Design Discount
				</div>
			</div>
			<div class="card-body m-0 p-1">
				<div class="row m-0 mb-3">
					...
					<div
						v-if="
							orderLine.tracingCharge &&
								orderLine.creativeCharge &&
								orderLine.scaledArtCharge &&
								orderLine.colourWashCharge
						"
			...
			<div class="card-footer p-1">
				...
				<div class="float-right">
					<div class="text-center p-1" v-if="orderLine.totalAddOns > 0">
						Sub Total: ${{ formatPrice(totalBeforeAddOns) }}
					</div>
					<div
						class="rounded bg-dark text-center text-white p-1"
						v-if="orderLine.totalAddOns > 0"
					>
						Total Add-Ons: ${{ formatPrice(orderLine.totalAddOns) }}
					</div>
					<div
						class="rounded bg-secondary text-center text-light p-2"
						style="font-size: 16px; font-weight: bold;"
					>
						Line Total: $ {{ formatPrice(orderLine.itemsSubTotal) }}
					</div>
				</div>
			</div>
		</div>
