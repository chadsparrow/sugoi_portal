<template>
	<div
		class="item card border-dark mb-2 nobreak"
		v-if="item.cancelled === false"
	>
		<div class="card-header bg-info text-light p-1">
			<div class="row align-items-center m-0">
				<div
					class="col-sm-1 d-print-none align-items-center ml-3"
		...
		<div class="card-body p-1">
			...
			<div class="row m-0 p-0">
				<div class="col text-center" v-if="orderLine.graphicColours > 0">
					<div class="row">
						<div v-if="item.colour1" class="col-sm-12">
							Colour 1: {{ item.colour1 }}
						</div>
						<div v-if="item.colour2" class="col-sm-12">
							Colour 2: {{ item.colour2 }}
						</div>
						<div v-if="item.colour3" class="col-sm-12">
							Colour 3: {{ item.colour3 }}
						</div>
					</div>
				</div>
				...
				<div class="col text-center">
					...
					<br />
					<span style="font-weight: bold; font-size: 14px;"
						>$ {{ formatPrice(item.finalUnitPrice) }}</span
					>
				</div>
			</div>
			<div class="card-footer p-1">
				<div class="float-left m-0 d-print-none" v-if="disabledEdit === false">
					...
					>
					<button class="btn btn-sm btn-danger" @click.prevent="cancelItem">
						Cancel Item
					</button>
				</div>
				<div
					class="row text-center float-right m-0"
...
export default {
	...
	computed: {
		...
		discountTotal() {
			if (this.item.itemDiscountType === '$') {
				return (
					(this.item.itemDiscount + this.qdDiscount) * this.item.totalUnits
				);
			}
